...
bool f(vi& wt, int mh, ll m){
    ll cnt = 0;
    for(auto i: wt){
        ll q = (sqrt((8*1ll*(m*1.0/i)+1))-1)/2;
        // if(m==12){ debug(make_pair(q, cnt))}
        cnt += q;
    }
    // debug(make_pair(m, cnt))
    return cnt>=mh;
}
...
long long minNumberOfSeconds(int mh, vector<int>& wt) {
        sort(all(wt));
        ll left = 0, right = 1;
        while(!f(wt, mh, right)) right = right*1ll*2;
        // debug(right)
        while(left<=right){
            ll mid = (left+right)/2;
            if(f(wt, mh, mid)){
