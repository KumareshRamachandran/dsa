...
bool f(vector<ll> &v, int n, ll sum, ll mid){
    double avg = (sum+mid)*1.0/(2*n); int cnt = 0;
    v[max_element(all(v))-v.begin()] += mid;
    for(auto i: v){
        if(i<avg) cnt++;
    }
    return cnt>n/2;
...
void solve()
{
    ...
    ll left = 0, right = 1;
    while(!f(v, n, sum, right)) right = right*2ll;
    while(left<=right){
        ll mid = (left+right)/2;
        if(f(v, n, sum, mid)){
            right = mid-1;
